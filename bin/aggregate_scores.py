...
def load_digest_tsvs(path):
    ...
    if df_min_max_scaled is not None:
        ...
        df_aggregated = aggregate_scores(df_min_max_scaled, scaled_cols=scaled_cols, output_col=f"score_{name_method}")
...
def load_seed_perturbation_tsv(path):
    ...
    col_to_keep = [df.columns[0], "avg_jaccard_index_scaled", "normalized_rediscovery_rate_scaled"]
    
    if df_scaled is not None:
        for col in ["avg_jaccard_index_scaled", "normalized_rediscovery_rate_scaled"]:
            if col not in scaled_cols:
                print(f"Warning: {col} from file {path} was not scaled and will not be included in the output.")
    ...
    if df_scaled is not None:
        ...
        for col in scaled_cols:
            if col in df_scaled.columns:
                new_col_name = f"score_network_perturbation_{col.replace('_scaled', '')}"
